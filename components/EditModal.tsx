...
export default function EditModal({ item, categories, onSave, onClose }: Props) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div>
            <label className="text-xs font-medium text-gray-500 block mb-1.5">가격</label>
            <input
              value={price}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full h-9 px-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20"
              >
            ...
            <div>
              ...
              <select
                value={status}
                onChange={e => setStatus(e.target.value as LinkItem['status'])}
                className="w-full h-9 px-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20"
              >
        ...
        <div className="px-5 pb-4 flex gap-2">
          <button onClick={onClose} className="flex-none h-9 px-4 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600">
            취소
          </button>
          <button onClick={handleSave} className="flex-1 h-9 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
            저장
          </button>
        </div>
      </div>
