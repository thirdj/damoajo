...
// base64 이미지는 렌더링에서 제외 (DB 정리 전까지)
function safeThumb(thumb: string | null): string | null {
  if (!thumb) return null
  if (thumb.startsWith('data:')) return null // base64 제외
  return thumb
}

function ThumbImage({ src, alt, fill, width, height, className }: {
  src: string; alt: string; fill?: boolean; width?: number; height?: number; className?: string
}) {
  const [error, setError] = useState(false)
  if (error) return null
  if (fill) {
    return <Image src={src} alt={alt} fill className={className} unoptimized loading="lazy" onError={() => setError(true)} />
  }
  return <Image src={src} alt={alt} width={width!} height={height!} className={className} unoptimized loading="lazy" onError={() => setError(true)} />
}
...
export default function LinkCard({ item, view, onEdit, onDelete, onPriceHistory, onToggleFavorite }: Props) {
  ...
  const thumbnail = safeThumb(item.thumbnail)
    ...
    return (
      <div className="flex items-center gap-3 bg-white border border-gray-100 rounded-xl px-3 py-2.5 hover:border-gray-200 transition-colors group">
        <div className="w-12 h-12 rounded-lg bg-gray-50 flex-shrink-0 overflow-hidden flex items-center justify-center">
          {thumbnail
            ? <ThumbImage src={thumbnail} alt="" width={48} height={48} className="object-cover w-full h-full" />
            : item.favicon
              ? <ThumbImage src={item.favicon} alt="" width={24} height={24} />
        ...
        <div className="flex-1 min-w-0">
          <a href={item.url} target="_blank" rel="noopener noreferrer"
            ...
            style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: '1.35' }}>
            {item.title}
          </a>
          <div className="flex items-center justify-between mt-0.5">
            ...
            <div className="flex items-center gap-0.5 flex-shrink-0">
              <button onClick={() => onToggleFavorite(item.id, !item.is_favorite)}
                className={`p-1 rounded-lg transition-colors ${item.is_favorite ? 'text-yellow-400' : 'text-gray-300 opacity-0 group-hover:opacity-100'}`}>
    ...
    <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden hover:border-gray-200 hover:shadow-md transition-all group flex flex-col relative">
      <button onClick={() => onToggleFavorite(item.id, !item.is_favorite)}
        className={`absolute top-2 right-2 z-10 p-1.5 rounded-full backdrop-blur-sm shadow-sm transition-all ${
          item.is_favorite ? 'bg-yellow-400 text-white' : 'bg-white/80 text-gray-400 opacity-0 group-hover:opacity-100'
        }`}>
      ...
      <a href={item.url} target="_blank" rel="noopener noreferrer" className="block flex-shrink-0">
        <div className="relative aspect-[4/3] bg-gray-50 overflow-hidden">
          {thumbnail ? (
            <ThumbImage src={thumbnail} alt={item.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
          ) : (
            <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-gradient-to-br from-gray-50 to-gray-100">
              {item.favicon ? (
                <>
                  <ThumbImage src={item.favicon} alt="" width={36} height={36} className="rounded-lg shadow-sm" />
      ...
      <div className="p-3 flex flex-col flex-1">
        <div className="flex items-center gap-1 mb-1">
          {item.favicon && <ThumbImage src={item.favicon} alt="" width={11} height={11} className="rounded-sm opacity-60" />}
        ...
        <div className="mb-2" style={{ minHeight: '2.6rem' }}>
          <a href={item.url} target="_blank" rel="noopener noreferrer"
            className="text-sm font-semibold text-gray-900 hover:text-blue-600 transition-colors"
            style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: '1.3rem' }}>
            {item.title}
          </a>
        </div>
        ...
        {displayPrice && (
          <button onClick={onPriceHistory} className="text-left mb-2">
            ...
            {hasPriceChange && <span className="ml-1.5 text-[10px] text-green-600 bg-green-50 px-1.5 py-0.5 rounded-full">↓변동</span>}
          </button>
        )}
